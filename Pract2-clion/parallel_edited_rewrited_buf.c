...
//        partOfX               = (double *) malloc(sizeof(double) * partsVector[mpiRank]);
        ...
        while (iterationsCounter < 10000 && iterationVectorScalarSquare >= newEps)
        {
            ...
            subVectors(x + positionsVector[mpiRank], partOfIterationVector, partOfIterationVector, partsVector[mpiRank]);

            MPI_Allgatherv(partOfIterationVector, partsVector[mpiRank], MPI_DOUBLE, x, partsVector, positionsVector, MPI_DOUBLE, MPI_COMM_WORLD);
